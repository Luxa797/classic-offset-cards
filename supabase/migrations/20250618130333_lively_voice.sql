/*
  # Fix order_summary_with_dues view - ensure balance_due column exists

  1. Changes
    - Drop and recreate the order_summary_with_dues view to ensure it has all required columns
    - Include the balance_due column that the frontend is expecting
    - Maintain all existing functionality and column definitions

  2. Notes
    - This fixes the error: "column order_summary_with_dues.balance_due does not exist"
    - Ensures the view matches what the DueSummary component expects
*/

-- Drop the existing view if it exists
DROP VIEW IF EXISTS public.order_summary_with_dues;

-- Create the view with all required columns including balance_due
